...
| 動線編輯 | 車隊面板「動線編輯」分頁：2D 畫布點選 / 拖曳編輯動線，存檔後熱套用（見下） |
...
**視覺化動線編輯器**（車隊面板 →「動線編輯」分頁，admin / operator 可編輯）

| 操作 | 說明 |
|---|---|
| 新增站點 | 在畫布空白處點擊；若已選取某站點，新點插入其後 |
| 移動站點 / 充電樁 | 直接拖曳 |
| 調整順序 | 右側屬性區 ▲▼ |
| 刪除站點 | 右側「刪除」或按 `Delete` |
| 站點屬性 | 站點名稱、x / y 座標、停留秒數、動作（移動／取貨／放貨／巡檢／充電／等待） |
| 車輛設定 | 車號、樓層、速度、循環 vs 原路折返；可新增 / 移除車輛 |
| 存檔 | 「儲存並套用」→ `PUT /api/robots/routes` → 後端驗證後立即熱套用 |

畫布為機器人原生導航坐標俯視圖（x 向右／y 向上，格線 1 m），並疊上該車的**實際即時位置**，
可直接比對規劃動線與實跑軌跡。送出前會先做與後端相同的驗證（點數、速度、座標範圍、動作、車號重複），
不通過時「儲存並套用」保持停用並顯示原因。編輯中的車輛動線會同步高亮於 3D 場景
（車隊監控頁的「動線與站點疊圖」可常駐顯示；高亮動線會穿透樓板以便對照）。

| 動線編輯器 | 3D 動線疊圖 |
|---|---|
| ![動線編輯器](docs/screenshots/13g_robot_route_editor.png) | ![動線疊圖](docs/screenshots/13h_robot_route_overlay.png) |
...
| 視覺化編輯器 | 車隊面板「動線編輯」分頁，點選拖曳後存檔（走同一支 API） |
